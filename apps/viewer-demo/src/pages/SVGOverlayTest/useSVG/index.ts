...
import { SVG_NAMESPACE, SVG_ROOT_ID } from '../../../../../../packages/renderer'
...
function useSVG() {
  ...
  useEffect(() => {
    ...
    const rootChildren = svgRoot?.children
    console.log(rootChildren?.length)

    if (svgData.length && !rootChildren?.length) {
      console.log('adding svg elements')
